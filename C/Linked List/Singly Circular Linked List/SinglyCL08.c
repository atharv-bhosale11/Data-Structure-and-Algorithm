...
{
    PNODE temp = NULL;                                  // with "temp" variable
    if(*first == NULL && *last == NULL)
    {
        return;
    }
...
2. First pointer is moved to the next node.
3. Last node is reconnected to the new first node.
4. Special handling is provided for:
      - Empty list
      - Single node list
5. Circular nature of the linked list is preserved.
