...
second node. The program displays the addresses of
both nodes and the pointer values stored in the
next members.

The arrow (->) operator is then used to access the
data member of the second node through the pointer
