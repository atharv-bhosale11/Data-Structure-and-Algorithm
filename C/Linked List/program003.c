...
Description     : This program demonstrates the creation of a singly
                  ...
                  and next members. It also shows how to access and
                  display node data using the dot operator.
...
Explanation :
A node object is created using the self-referential
structure. The data member is initialized with the
value 11 and the next pointer is assigned NULL,
indicating that no subsequent node exists.
